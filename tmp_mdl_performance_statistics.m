...
% 95% co-efficient level

%%  ROC curve
fprintf('ROC curve for learnt models ...\n\n');

% 1: generates ROC & PR curves 
% 0: doesn't generate ROC & PR curves
ROC_option = 0; 

if ROC_option == 1
    num_roc_division = 5;
    roc_bins = linspace(0.95, 1, num_roc_division+1);
    roc_bins = roc_bins (1 : end-1); 
    num_ROC_iter = num_roc_division - 1;
else
    roc_bins = 0.5;
    num_ROC_iter = 1;
end

roc_overall_detections = zeros(num_ROC_iter, num_mdl_2);

for m = 1 : num_mdl_2

    switch num_mdl_2
        case 1
            tmp_final_mdl_pred = test_pred_LR_final;
            tmp_final_mdl_score = test_scores_LR_final;
        case 1
            tmp_final_mdl_pred = test_pred_LR_final;
            tmp_final_mdl_score = test_scores_LR_final;
        case 1
        ...
        case 1
            ...
            tmp_final_mdl_score = test_scores_LR_final;
        otherwise
            disp('Selected model is out of range ...\n');
    end

    for r = 1 : num_ROC_iter

        fprintf('ROC curve iterations: %d / %d ...\n', r, num_ROC_iter);

        for j = 1:length(tmp_final_mdl_pred)
            % Convert test scores to classification probabilities
            if classifier_option == 2
                test_scores = sigmoid (tmp_final_mdl_score{j});
            else
                test_scores = tmp_final_mdl_score{j};
            end

            % Calculate the final test prediction
            tmp_final_mdl_pred{j} = test_scores(:,2) >= roc_bins(r);
        end
        ...
        elseif (data_div_option == 2)||(data_div_option == 3) % Based on stratified K-fold division
            [prediction_overall_dataset_TPD,prediction_overall_dataset_FPD] = ...
                get_overall_test_prediction(tmp_final_mdl_pred,n_TPD,n_data_TPD_each_fold,...
        ...
        elseif data_div_option == 4 % Based on data division by participants
            ...
            for i = 1:n_participant
                ...
                prediction_overall_dataset_TPD = [prediction_overall_dataset_TPD;tmp_final_mdl_pred{i}(1:n_TPD_current)];
                prediction_overall_dataset_FPD = [prediction_overall_dataset_FPD;tmp_final_mdl_pred{i}(n_TPD_current+1:end)];
            end
        ...
        roc_overall_detections(r,:) = [TPD_overall{1,1},FPD_overall{1,1},TND_overall{1,1},FND_overall{1,1}];
        ...
        % end of ROC iteration
    end

    % end of loop for each ML model
end
...
for i = n1 : n1
    ...
    if (n_label) % When there is a detection by the sensor system
        for r = 1 : n_label
            L_min = find(sensor_data_sgmntd_cmbd_all_sensors_labeled == r, 1 ); % Sample no. corresponding to the start of the label
            L_max = find(sensor_data_sgmntd_cmbd_all_sensors_labeled == r, 1, 'last' ); % Sample no. corresponding to the end of the label
