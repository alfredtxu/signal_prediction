...
for i = 1 : num_files
    ...
    for j2 = 1 : num_FMsensors
        ...
        FM_thresh(i, j2) = tmp_sensor_indiv_mu + tmp_std_times_FM * tmp_sensor_indiv_std;
        ...
        tmp_seg_thresh = (tmp_sensor_indiv >= FM_thresh(i, j2));
        tmp_seg_thresh = imdilate(tmp_seg_thresh, FM_lse); 
        [tmp_FM_seg_label, tmp_FM_seg_comp] = bwlabel(tmp_seg_thresh);

        tmp_seg_IMUa_thresh = tmp_seg_thresh;
        tmp_seg_IMUg_thresh = tmp_seg_thresh;
        tmp_seg_IMUag_thresh = tmp_seg_thresh;

        for f = 1 : tmp_FM_seg_comp
            
            tmp_idx = find(tmp_seg_thresh == f);

            tmp_overlaps_acc = sum(tmp_seg_thresh(tmp_idx) .* IMUa_map{i, :}(tmp_idx));
            if tmp_overlaps_acc >= overlap_perc * length(tmp_idx)
                tmp_seg_IMUa_thresh(tmp_idx) = 0;
                tmp_seg_IMUag_thresh(tmp_idx) = 0;
            end

            tmp_overlaps_gyr = sum(tmp_seg_thresh(tmp_idx) .* IMUg_map{i, :}(tmp_idx));
            if tmp_overlaps_gyr >= overlap_perc * length(tmp_idx)
                tmp_seg_IMUg_thresh(tmp_idx) = 0;
                tmp_seg_IMUag_thresh(tmp_idx) = 0;
            end

            clear tmp_idx tmp_overlaps_acc tmp_overlaps_gyr
        end

        FM_segmented{i, j2} = tmp_seg_thresh;
        FM_segmented_IMUa{i, j2} = tmp_seg_IMUa_thresh;
        FM_segmented_IMUg{i, j2} = tmp_seg_IMUg_thresh;
        FM_segmented_IMUag{i, j2} = tmp_seg_IMUag_thresh;

        clear tmp_sensor_indiv tmp_sensor_indiv_mu tmp_sensor_indiv_std ...
              tmp_FM_removed tmp_FM_segmented p_signal n_signal tmp_std_times_FM
    end
    % -------------------------------------------------------------------------
    ...
    match_sensor_sensation{i, :} = tmp_sensor_label .* sens_map{i, :};
    ...
    fprintf('Signal segmentation file %d >> comp (sens_IMUag / FM_IMUag): %d / %d; match: %d ...\n', ...
