...
for i = 1 : size(participants, 2)
    ...
    clear acceLNetFilTEq acceRNetFilTEq acouLFilTEq acouRFilTEq piezLFilTEq piezRFilTEq ...
          forcFilTEq IMUacceNetFilTEq sens1TEq sens2TEq sens1TEq_mtxP time_vecTEq ...
          tmp_mat
end
...
for i = 1 : num_files
    ...
    for n1 = 1 : length(Nstd_IMU_TB)
        ...
        tmp_IMUaTB = abs(all_IMUacce_cat{i, :});
        ...
        FM_suiteTB{i, 1} = all_acceL_cat{i,:};
        FM_suiteTB{i, 2} = all_acceR_cat{i,:};
        FM_suiteTB{i, 3} = all_acouL_cat{i,:};
        FM_suiteTB{i, 4} = all_acouR_cat{i,:};
        FM_suiteTB{i, 5} = all_piezL_cat{i,:};
        FM_suiteTB{i, 6} = all_piezR_cat{i,:};
...
cd(fdir);
save(['Seg01_butter_' group '_' data_category '_' sprintf('%03d',fportion_s) '_' sprintf('%03d',fportion_e) ...
      '_FMsnr' num2str(max(snr_acou_th)) ...
      '_IMUx' num2str(Nstd_IMU_TB) ...
      '_OLs' num2str(overlap_percSens*100) ...
      '_OLf' num2str(overlap_percFM*100) ...
      '_LN' num2str(noise_quan*100) ...
      '_LL' num2str(low_quan*100) ...
