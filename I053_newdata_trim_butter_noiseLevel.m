% the lowest 25% signals (FM sensors only) are removed before calculating the adaptive thresholds
clc
clear 
close all
...
for i = 1 : num_files
    ...
    for j2 = 1 : num_FMsensors
        ...
        tmp_sensor_indiv_ori = abs(FM_suiteTB{i, j2});
        tmp_low_cutoff = quantile(tmp_sensor_indiv_ori, low_quan); 
        tmp_sensor_indiv = tmp_sensor_indiv_ori;
        tmp_sensor_indiv(tmp_sensor_indiv<=tmp_low_cutoff, :) = 0;
        ...
        while tmp_SNR_sensor <= tmp_snr_FM
            ...
            tmp_FM_threshTB = mean(tmp_sensor_indiv) + tmp_std_times_FM * std(tmp_sensor_indiv);
