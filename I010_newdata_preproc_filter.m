...
for i = 1 : length(files)
    ...
    % Bandpass & Low-pass filtering applied on raw data
    forc_F{i, :} = filtfilt(sos_force, g_force, forc{i, :});
    acouL_F{i, :} = filtfilt(sos_FM, g_FM, acouL{i, :});
    acouM_F{i, :} = filtfilt(sos_FM, g_FM, acouM{i, :});
    acouR_F{i, :} = filtfilt(sos_FM, g_FM, acouR{i, :});
    piezL_F{i, :} = filtfilt(sos_FM, g_FM, piezL{i, :});
    piezM_F{i, :} = filtfilt(sos_FM, g_FM, piezM{i, :});
    piezR_F{i, :} = filtfilt(sos_FM, g_FM, piezR{i, :});
    IMUaccNet_F{i, :} = filtfilt(sos_IMU, g_IMU, IMUaccNet{i, :});
    IMUgyrNet_F{i, :} = filtfilt(sos_IMU, g_IMU, IMUgyrNet{i, :});
    ...
    % time vector & positive sensation
    duration_T{i, :} = ((0 : (length(forc_TF{i, :})-1)) / freq)';
    sens_T_mtx{i, :} = [duration_T{i, :}, sens_T{i, :}];
    
    % sensation button press sub-set
    sens_T_idxP{i, :} = sens_T{i, :}==1;
    sens_T_mtxP{i, :} = sens_T_mtx{i, :}(sens_T_idxP{i, :}, :);
...
forc_TF_cat = cat(1, forc_TF{:});
acouL_TF_cat = cat(1, acouL_TF{:});
acouM_TF_cat = cat(1, acouM_TF{:});
acouR_TF_cat = cat(1, acouR_TF{:});
piezL_TF_cat = cat(1, piezL_TF{:});
piezM_TF_cat = cat(1, piezM_TF{:});
piezR_TF_cat = cat(1, piezR_TF{:});
sens_T_cat = cat(1, sens_T{:});
IMUaccNet_TF_cat = cat(1, IMUaccNet_TF{:});
IMUgyrNet_TF_cat = cat(1, IMUgyrNet_TF{:});
...
save(['FM01_' data_folder '_' data_category '_preproc.mat'], ...
      ...
      'forc_F', 'acouL_F', 'acouM_F', 'acouR_F', 'piezL_F', 'piezM_F', 'piezR_F', 'IMUaccNet_F', 'IMUgyrNet_F', ...
      'forc_TF', 'acouL_TF', 'acouM_TF', 'acouR_TF', 'piezL_TF', 'piezM_TF', 'piezR_TF', 'IMUaccNet_TF', 'IMUgyrNet_TF', ...
      'duration_T', 'sens_T_mtx', 'sens_T_idxP', 'sens_T_mtxP', ...
      '-v7.3' );
save(['FM01_' data_folder '_' data_category '_preproc_cat.mat'], ...
      'forc_TF_cat', 'acouL_TF_cat', 'acouM_TF_cat', 'acouR_TF_cat', 'piezL_TF_cat', 'piezM_TF_cat', 'piezR_TF_cat', 'IMUaccNet_TF_cat', 'IMUgyrNet_TF_cat', ...
      '-v7.3' );
cd(curr_dir)
