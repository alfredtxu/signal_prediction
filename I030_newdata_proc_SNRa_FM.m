...
for i = 1 : num_files
    ...
    for t = 1 : length(FM_min)
        ...
        for j2 = 1 : num_FMsensors
            ...
            FM_thresh(i, t, j2) = FM_min(t) * median(tmp_sensor_suite_low);
            ...
            if FM_thresh(i, t, j2) < noise_level
                FM_thresh(i, t, j2) = Inf; 
            end
            ...
            fprintf('Current data file from %s (%s): %d of %d >> FM threshold: %d - SNR: %d - ratio: %d ...\n', ...
                data_folder, data_category, i, num_files, FM_min(t), SNR(i, t, j2), length(tmp_FM_removed) / length(tmp_FM_segmented));
